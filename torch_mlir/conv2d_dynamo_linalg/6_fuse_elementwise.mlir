module attributes {torch.debug_module_name = "Conv2dWithPaddingDilationStrideModule"} {
  ...
  func.func @forward(%arg0: tensor<?x?x?x?xf32>) -> tensor<?x10x?x?xf32> {
    ...
    %dim_3 = tensor.dim %arg0, %c1 : tensor<?x?x?x?xf32>
    %0 = arith.addi %dim_1, %c6 : index
    %1 = arith.addi %dim_2, %c6 : index
    %2 = tensor.empty(%dim, %dim_3, %0, %1) : tensor<?x?x?x?xf32>
    %3 = linalg.fill ins(%cst_0 : f32) outs(%2 : tensor<?x?x?x?xf32>) -> tensor<?x?x?x?xf32>
    %inserted_slice = tensor.insert_slice %arg0 into %3[0, 0, 3, 3] [%dim, %dim_3, %dim_1, %dim_2] [1, 1, 1, 1] : tensor<?x?x?x?xf32> into tensor<?x?x?x?xf32>
