module attributes {torch.debug_module_name = "Conv2dWithPaddingDilationStrideModule"} {
  memref.global "private" @global_seed : memref<i64> = dense<0>
  memref.global "private" constant @__constant_10x2x3x3xf32 : memref<10x2x3x3xf32> = dense<"">
  func.func @forward(%arg0: memref<?x?x?x?xf32>) -> memref<?x10x?x?xf32> {
    %c6 = arith.constant 6 : index
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c2 = arith.constant 2 : index
    %c3 = arith.constant 3 : index
    %0 = memref.get_global @__constant_10x2x3x3xf32 : memref<10x2x3x3xf32>
    %cst = arith.constant 0.000000e+00 : f32
    %c1_i64 = arith.constant 1 : i64
    %c2_i64 = arith.constant 2 : i64
    %dim = memref.dim %arg0, %c0 : memref<?x?x?x?xf32>
    %dim_0 = memref.dim %arg0, %c2 : memref<?x?x?x?xf32>
    %dim_1 = memref.dim %arg0, %c3 : memref<?x?x?x?xf32>
    %dim_2 = memref.dim %arg0, %c1 : memref<?x?x?x?xf32>
    %1 = arith.addi %dim_0, %c6 : index
    %2 = arith.addi %dim_1, %c6 : index
    %alloc = memref.alloc(%dim, %dim_2, %1, %2) {alignment = 64 : i64} : memref<?x?x?x?xf32>
    linalg.fill ins(%cst : f32) outs(%alloc : memref<?x?x?x?xf32>)
    %c0_3 = arith.constant 0 : index
    %dim_4 = memref.dim %alloc, %c0_3 : memref<?x?x?x?xf32>
    %c1_5 = arith.constant 1 : index
    %dim_6 = memref.dim %alloc, %c1_5 : memref<?x?x?x?xf32>
    %c2_7 = arith.constant 2 : index
    %dim_8 = memref.dim %alloc, %c2_7 : memref<?x?x?x?xf32>
    %c3_9 = arith.constant 3 : index
    %dim_10 = memref.dim %alloc, %c3_9 : memref<?x?x?x?xf32>
    %alloc_11 = memref.alloc(%dim_4, %dim_6, %dim_8, %dim_10) {alignment = 64 : i64} : memref<?x?x?x?xf32>
    memref.copy %alloc, %alloc_11 : memref<?x?x?x?xf32> to memref<?x?x?x?xf32>
    memref.dealloc %alloc : memref<?x?x?x?xf32>
    %subview = memref.subview %alloc_11[0, 0, 3, 3] [%dim, %dim_2, %dim_0, %dim_1] [1, 1, 1, 1] : memref<?x?x?x?xf32> to memref<?x?x?x?xf32, strided<[?, ?, ?, 1], offset: ?>>
    memref.copy %arg0, %subview : memref<?x?x?x?xf32> to memref<?x?x?x?xf32, strided<[?, ?, ?, 1], offset: ?>>
    %3 = arith.index_cast %dim_0 : index to i64
    %4 = arith.subi %3, %c1_i64 : i64
    %5 = arith.floordivsi %4, %c2_i64 : i64
    %6 = arith.addi %5, %c1_i64 : i64
    %7 = arith.index_cast %6 : i64 to index
    %8 = arith.index_cast %dim_1 : index to i64
    %9 = arith.subi %8, %c1_i64 : i64
    %10 = arith.floordivsi %9, %c2_i64 : i64
    %11 = arith.addi %10, %c1_i64 : i64
    %12 = arith.index_cast %11 : i64 to index
    %alloc_12 = memref.alloc(%dim, %7, %12) {alignment = 64 : i64} : memref<?x10x?x?xf32>
    linalg.fill ins(%cst : f32) outs(%alloc_12 : memref<?x10x?x?xf32>)
    %c0_13 = arith.constant 0 : index
    %dim_14 = memref.dim %alloc_12, %c0_13 : memref<?x10x?x?xf32>
    %c2_15 = arith.constant 2 : index
    %dim_16 = memref.dim %alloc_12, %c2_15 : memref<?x10x?x?xf32>
    %c3_17 = arith.constant 3 : index
    %dim_18 = memref.dim %alloc_12, %c3_17 : memref<?x10x?x?xf32>
    %alloc_19 = memref.alloc(%dim_14, %dim_16, %dim_18) {alignment = 64 : i64} : memref<?x10x?x?xf32>
    memref.copy %alloc_12, %alloc_19 : memref<?x10x?x?xf32> to memref<?x10x?x?xf32>
    memref.dealloc %alloc_12 : memref<?x10x?x?xf32>
    linalg.conv_2d_nchw_fchw {dilations = dense<3> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%alloc_11, %0 : memref<?x?x?x?xf32>, memref<10x2x3x3xf32>) outs(%alloc_19 : memref<?x10x?x?xf32>)
    memref.dealloc %alloc_11 : memref<?x?x?x?xf32>
    return %alloc_19 : memref<?x10x?x?xf32>
  }
}
