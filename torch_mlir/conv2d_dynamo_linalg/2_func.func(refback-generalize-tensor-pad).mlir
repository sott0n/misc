module attributes {torch.debug_module_name = "Conv2dWithPaddingDilationStrideModule"} {
  ml_program.global private mutable @global_seed(dense<0> : tensor<i64>) : tensor<i64>
  func.func @forward(%arg0: tensor<?x?x?x?xf32>) -> tensor<?x10x?x?xf32> {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c2 = arith.constant 2 : index
    %c3 = arith.constant 3 : index
    %cst = arith.constant dense<""> : tensor<10x2x3x3xf32>
    %cst_0 = arith.constant 0.000000e+00 : f32
    %c1_i64 = arith.constant 1 : i64
    %c2_i64 = arith.constant 2 : i64
    %dim = tensor.dim %arg0, %c0 : tensor<?x?x?x?xf32>
    %dim_1 = tensor.dim %arg0, %c2 : tensor<?x?x?x?xf32>
    %dim_2 = tensor.dim %arg0, %c3 : tensor<?x?x?x?xf32>
    %dim_3 = tensor.dim %arg0, %c0 : tensor<?x?x?x?xf32>
    %dim_4 = tensor.dim %arg0, %c1 : tensor<?x?x?x?xf32>
    %dim_5 = tensor.dim %arg0, %c2 : tensor<?x?x?x?xf32>
    %0 = arith.addi %dim_5, %c3 : index
    %1 = arith.addi %0, %c3 : index
    %dim_6 = tensor.dim %arg0, %c3 : tensor<?x?x?x?xf32>
    %2 = arith.addi %dim_6, %c3 : index
    %3 = arith.addi %2, %c3 : index
    %4 = tensor.empty(%dim_3, %dim_4, %1, %3) : tensor<?x?x?x?xf32>
    %5 = linalg.fill ins(%cst_0 : f32) outs(%4 : tensor<?x?x?x?xf32>) -> tensor<?x?x?x?xf32>
    %dim_7 = tensor.dim %arg0, %c0 : tensor<?x?x?x?xf32>
    %dim_8 = tensor.dim %arg0, %c1 : tensor<?x?x?x?xf32>
    %dim_9 = tensor.dim %arg0, %c2 : tensor<?x?x?x?xf32>
    %dim_10 = tensor.dim %arg0, %c3 : tensor<?x?x?x?xf32>
    %inserted_slice = tensor.insert_slice %arg0 into %5[%c0, %c0, %c3, %c3] [%dim_7, %dim_8, %dim_9, %dim_10] [1, 1, 1, 1] : tensor<?x?x?x?xf32> into tensor<?x?x?x?xf32>
    %6 = arith.index_cast %dim_1 : index to i64
    %7 = arith.subi %6, %c1_i64 : i64
    %8 = arith.floordivsi %7, %c2_i64 : i64
    %9 = arith.addi %8, %c1_i64 : i64
    %10 = arith.index_cast %9 : i64 to index
    %11 = arith.index_cast %dim_2 : index to i64
    %12 = arith.subi %11, %c1_i64 : i64
    %13 = arith.floordivsi %12, %c2_i64 : i64
    %14 = arith.addi %13, %c1_i64 : i64
    %15 = arith.index_cast %14 : i64 to index
    %16 = tensor.empty(%dim, %10, %15) : tensor<?x10x?x?xf32>
    %17 = linalg.fill ins(%cst_0 : f32) outs(%16 : tensor<?x10x?x?xf32>) -> tensor<?x10x?x?xf32>
    %18 = linalg.conv_2d_nchw_fchw {dilations = dense<3> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%inserted_slice, %cst : tensor<?x?x?x?xf32>, tensor<10x2x3x3xf32>) outs(%17 : tensor<?x10x?x?xf32>) -> tensor<?x10x?x?xf32>
    return %18 : tensor<?x10x?x?xf32>
  }
}
