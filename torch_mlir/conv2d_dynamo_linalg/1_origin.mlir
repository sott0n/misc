module attributes {torch.debug_module_name = "Conv2dWithPaddingDilationStrideModule"} {
  ml_program.global private mutable @global_seed(dense<0> : tensor<i64>) : tensor<i64>
  func.func @forward(%arg0: tensor<?x?x?x?xf32>) -> tensor<?x10x?x?xf32> {
    %cst = arith.constant dense<""> : tensor<10x2x3x3xf32>
    %c2 = arith.constant 2 : index
    %cst_0 = arith.constant 0.000000e+00 : f32
    %c1_i64 = arith.constant 1 : i64
    %c0 = arith.constant 0 : index
    %c3 = arith.constant 3 : index
    %c2_i64 = arith.constant 2 : i64
    %dim = tensor.dim %arg0, %c0 : tensor<?x?x?x?xf32>
    %dim_1 = tensor.dim %arg0, %c2 : tensor<?x?x?x?xf32>
    %dim_2 = tensor.dim %arg0, %c3 : tensor<?x?x?x?xf32>
    %padded = tensor.pad %arg0 low[%c0, %c0, %c3, %c3] high[%c0, %c0, %c3, %c3] {
    ^bb0(%arg1: index, %arg2: index, %arg3: index, %arg4: index):
      tensor.yield %cst_0 : f32
    } : tensor<?x?x?x?xf32> to tensor<?x?x?x?xf32>
    %0 = arith.index_cast %dim_1 : index to i64
    %1 = arith.subi %0, %c1_i64 : i64
    %2 = arith.floordivsi %1, %c2_i64 : i64
    %3 = arith.addi %2, %c1_i64 : i64
    %4 = arith.index_cast %3 : i64 to index
    %5 = arith.index_cast %dim_2 : index to i64
    %6 = arith.subi %5, %c1_i64 : i64
    %7 = arith.floordivsi %6, %c2_i64 : i64
    %8 = arith.addi %7, %c1_i64 : i64
    %9 = arith.index_cast %8 : i64 to index
    %10 = tensor.empty(%dim, %4, %9) : tensor<?x10x?x?xf32>
    %11 = linalg.fill ins(%cst_0 : f32) outs(%10 : tensor<?x10x?x?xf32>) -> tensor<?x10x?x?xf32>
    %12 = linalg.conv_2d_nchw_fchw {dilations = dense<3> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%padded, %cst : tensor<?x?x?x?xf32>, tensor<10x2x3x3xf32>) outs(%11 : tensor<?x10x?x?xf32>) -> tensor<?x10x?x?xf32>
    return %12 : tensor<?x10x?x?xf32>
  }
}